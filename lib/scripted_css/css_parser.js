...
break;
case 58:this.$ = new yy.StringNode($$[$0]).content();
break;
case 59:this.$ = $$[$0];
break;
case 60:this.$ = $$[$0-1] + $$[$0];
...
lexer.rules = [/^url/,/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})\b/,/^-?\d+(\.\d+)?(%|in|cm|mm|em|ex|pt|pc|px|ms|s)/,/^-?\.\d+(%|in|cm|mm|em|ex|pt|pc|px|ms|s)/,/^-?\d+(\.\d+)?/,/^-?\.\d+/,/^[.#]?[a-zA-Z_-][a-zA-Z0-9_-]*[.#][a-zA-Z0-9.#_-]+/,/^[a-zA-Z_-][a-zA-Z0-9_-]*/,/^("[^"]*"|'[^']*')/,/^\s+/,/^\/\*.*?\*\//,/^\@/,/^\$/,/^\+/,/^\{/,/^\}/,/^\(/,/^\)/,/^\[/,/^\]/,/^\</,/^\>/,/^\^/,/^\?/,/^\&/,/^\!/,/^\~/,/^=/,/^\-/,/^\|/,/^\#/,/^\./,/^::/,/^:/,/^;/,/^\*/,/^\\/,/^\//,/^,/];
...
if (typeof module !== 'undefined' && require.main === module) {
  ...
  var AttributeNode, AttributeSelectorNode, CssAST, FunctionNode, LiteralNode, MetaNode, MetaSelectorNode, MultiLiteral, RuleNode, RulesNode, SelectorNode, StringNode, collectStrings;
  ...
  CssAST = {
    RulesNode: RulesNode = (function() {
      ...
      RulesNode.prototype.index = function() {
        var meta, rule, _i, _len, _ref, _results;
        meta = {};
        _ref = this.rules;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          rule = _ref[_i];
          _results.push(rule.selectors != null ? (rule.meta = meta, this.indexRule(rule)) : (meta[rule.name] = rule.value, this.metaRules.push(rule)));
        }
        return _results;
      };
      RulesNode.prototype.indexRule = function(rule) {
        var attribute, _base, _i, _len, _name, _ref, _ref2;
        _ref = rule.attributes;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          attribute = _ref[_i];
          attribute.rule = rule;
          (_ref2 = (_base = this.attributes)[_name = attribute.name]) != null ? _ref2 : _base[_name] = [];
          this.attributes[attribute.name].push(attribute);
        }
        return this.elementRules.push(rule);
      };
    ...
    StringNode: StringNode = (function() {
      ...
      StringNode.prototype.content = function() {
